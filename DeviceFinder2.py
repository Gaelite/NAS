...
if __name__ == "__main__":
            ...
            for x in range(len(neighbor_device)):
                ...
                for z in all_devices[deviceNum].interfaces:
                    if z["Interface"] == neighbor_device[x][5] or z["Interface"] == "Vlan1":
                        print(z["Interface"], '==', neighbor_device[x],':',z["IPv4"])
                        local_Ip = z["IPv4"]
    ...
    for x in all_devices:
        interfaces = [{"Interface": i["Interface"], "IPv4": i["IPv4"], "IPv6": i["IPv6"], "Link-local": i["Link-local"]} for i in x.interfaces]
        connections = [{"Connected_from_Interface": c["Connected_from_Interface"], "From_IP": c["From_IP"], "Device": c["Device"], "Connected_to_Interface": c["Connected_to_Interface"], "To_IP": c["To_IP"]} for c in x.connections]
        json_Pack.append({
